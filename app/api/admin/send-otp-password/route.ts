...
export async function POST(request: NextRequest) {
  try {
    const { newPassword, userId, method = "email" } = await request.json()
    ...
    if (!userData || (!userData.email && !userData.phone)) {
      return NextResponse.json({ 
        error: "Admin email or phone not found." 
      }, { status: 400 })
    }
    ...
    await db.collection("admin_otp").doc("password_change").set({
      otp,
      newPassword,
      userId,
      type: "password",
    ...
    if (method === "sms") {
      ...
      try {
        await sendSMS(
          String(adminPhone),
          `Il tuo codice di verifica per modificare la password è: ${otp}. Valido per 10 minuti.`
        )
        
        console.log(`[v0] Sent Password change OTP via SMS to ${adminPhone}: ${otp}`)
      ...
      try {
        await resend.emails.send({
          ...
          to: adminEmail,
          subject: "Codice di Verifica - Cambio Password",
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #333;">Codice di Verifica</h2>
              <p>Hai richiesto di modificare la password del tuo account amministratore.</p>
        ...
        console.log(`[v0] Sent Password change OTP via Email to ${adminEmail}: ${otp}`)
